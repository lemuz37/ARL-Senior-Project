...
namespace UnBox3D.Utils
{
    ...
    public class Logger : ILogger
    {
        ...
        public Logger(IFileSystem fileSystem, string logDirectory = null, string logFileName = "UnBox3D.log", long maxFileSizeInBytes = 5 * 1024 * 1024, int maxArchiveFiles = 5)
        {
            _fileSystem = fileSystem ?? throw new ArgumentNullException(nameof(fileSystem));
            _logDirectory = logDirectory ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "UnBox3D", "Log");
        ...
        public void Log(string message, LogLevel level = LogLevel.Info)
        {
            lock (_lock)
            {
                if (_fileSystem.DoesFileExists(_logFilePath) && _fileSystem.GetFileSize(_logFilePath) > _maxFileSizeInBytes)
                    RotateLogs();
